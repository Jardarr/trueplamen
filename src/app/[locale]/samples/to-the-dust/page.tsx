...
import toTheDust from "./toTheDust.utils";
...
export const metadata: Metadata = {
	title: "Plamen | To The Dust",
	description: "To The Dust. You have come into this world to blaze in the sun of the City... And the fiercer the evil, the brighter burns the flame of pure hearts!",
...
const ToTheDust = () => {
	...
	const album = toTheDust[0];
	const audio = album.audio ?? "";
	const audioTitle = album.audioTitle ?? "";
	const tracklist = t("Release.ToTheDust.tracklist");
		...
		<div className="custom-font relative w-full flex flex-col items-center gap-8 pt-10 sm:justify-center pb-20 bg-main-bg bg-cover bg-fixed bg-top bg-no-repeat min-h-screen sm:h-fit">
			<div className="max-w-[900px] w-full flex flex-col sm:flex-row">
				<div className="flex flex-col items-center bg-neutral-800/45 p-3 rounded-md">
					...
					<h1 className="text-gray-300 text-base">{t(`Release.ToTheDust.title`)}</h1>
					<h2 className="text-gray-400 text-sm text-center font-thin">{t("Release.ToTheDust.description")}</h2>
				</div>
