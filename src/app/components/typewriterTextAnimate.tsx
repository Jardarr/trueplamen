...
export default function TypewriterTextAnimate() {
	...
	useEffect(() => {
		...
		const hasAnimated = sessionStorage.getItem("typewriterPlayed");

		if (hasAnimated) {
			textRef.current.innerHTML = "Вы пришли в мир, чтоб гореть в солнце Града…<br />А чем лютей зло, тем ярче пламень чистых сердец!";
			gsap.set(iconRef.current, { opacity: 1, y: 0 });
			gsap.set(blockRef.current, { opacity: 1, scaleY: 1 });
			return;
		}
